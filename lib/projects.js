 ...
 * @param  {String}   projectSlug Project slug.
 * @param  {Object}   query
 * @param  {String}   query.statsPeriod An optional stat period (can be one of "24h", "14d", and "").
 * @param  {String}   query.query An optional Sentry structured search query. If not provided an implied "is:resolved" is assumed.)
 * @param  {Function} callback
 * @return {Promise}
 */
Projects.prototype.groups = function(orgSlug, projectSlug, query, callback) {
  var path = util.format('projects/%s/%s/groups/', orgSlug, projectSlug);
  return this.client.get(path, query, callback);
}
 ...
 * @return {Promise}
 */
Projects.prototype.events = function(orgSlug, projectSlug, query, callback) {
  var path = util.format('projects/%s/%s/events/', orgSlug, projectSlug);
  return this.client.get(path, query, callback);
}
